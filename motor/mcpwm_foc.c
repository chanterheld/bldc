...
static void timer_reinit(int f_zv) {
	...
	TIM_TimeBaseStructure.TIM_Period = (SYSTEM_CORE_CLOCK / (f_zv * HW_LOBSTER_FVZ_UPSCALE_FACTOR));
	...
	TIM_TimeBaseStructure.TIM_Period = HW_LOBSTER_FVZ_UPSCALE_FACTOR - 1;
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	...
	TIM_ITConfig(TIM2, TIM_IT_Update, ENABLE); //now trigger on update events, like ADC
	utils_sys_unlock_cnt();
...
void mcpwm_foc_init(mc_configuration *conf_m1, mc_configuration *conf_m2) {
	...
	ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T2_TRGO; //config so generated by update event
...
void mcpwm_foc_set_configuration(mc_configuration *configuration) {
	...
	uint32_t top = (SYSTEM_CORE_CLOCK / ((int)configuration->foc_f_zv * HW_LOBSTER_FVZ_UPSCALE_FACTOR));
	if (TIM1->ARR != top) {
#ifdef HW_HAS_DUAL_MOTORS
		m_motor_1.m_control_mode = CONTROL_MODE_NONE;
		m_motor_1.m_state = MC_STATE_OFF;
